...
bool UART_Init(const uint32_t moduleClk, const uint32_t baudRate)
{
	...
	CLOCK_Enable(kCLOCK_Uart0);
	CLOCK_Enable(kCLOCK_PortB); // Enable clock to portB so we can configure it

	PORT_SetPinConfig(PORTB, 16, &UART_PORT_PIN_CONFIG); //CMSYS command??
