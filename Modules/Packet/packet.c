...
static uint8_t currentState = 0; // current state
...
bool Packet_Get(void){
	...
	case 0:
		if(UART_InChar(&Packet_Command)) // true if packet received from RxFIFO
			state = 1; // currentState value will be changed to 1 if true
		else
			return false;
		break;
	case 1:
		if(UART_InChar(&Packet_Parameter1))  // true if packet received from RxFIFO
			state = 2; //currentState value will be changed to 2 if true
		else
			return false;
		break;
	case 2:
			if(UART_InChar(&Packet_Parameter2))  // true if packet received from RxFIFO
				state = 3; //currentState value will be changed to 3 if true
			else
				return false;
			break;
	case 3:
			if(UART_InChar(&Packet_Parameter3))  // true if packet received from RxFIFO
				state = 4; //currentState value will be changed to 4 if true
			else
				return false;
			break;
	case 4:
			if(UART_InChar(&Packet_Checksum))  // true if packet received from RxFIFO
				state = 5; //currentState value will be changed to 5 if true
			else
				return false;
			break;
	case 5:
		...
		if((Packet_Command ^ Packet_Parameter1 ^ Packet_Parameter2 ^ Packet_Parameter3) == Packet_Checksum){
			...
			state = 4; // go to state 4 if packet not valid
			return false;
		}
...
bool Packet_Put(const uint8_t command, const uint8_t parameter1, const uint8_t parameter2, const uint8_t parameter3){
	...
	//packets sent byte by byte to TxFIFO
	UART_OutChar(command);
	UART_OutChar(parameter1);
	UART_OutChar(parameter2);
	UART_OutChar(parameter3);
	UART_OutChar(command ^ parameter1 ^ parameter2 ^ parameter3 ); // value of checksum XORed before sending data
